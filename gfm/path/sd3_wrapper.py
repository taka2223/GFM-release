...
def run_gfm_interpolation(
    ...
    for t in range(T):
        optimized_path[:, t] = SD3GFMPipeline.project_to_sphere(
    ...
    # 8. Backward denoise
    print(f"Flow backward ({n_frames} frames)...")
    output_latents_clean, output_images = [], []
    for t_idx in tqdm(range(n_frames), desc="Denoising"):
        ...
        lat_clean = sd3_pipe.flow_backward(
            output_latents_noisy[:, t_idx], embed_t, pooled_t, noise_level, cfg_scale
        )
        output_latents_clean.append(lat_clean)
        output_images.append(sd3_pipe.autoencoder.decode(lat_clean))
